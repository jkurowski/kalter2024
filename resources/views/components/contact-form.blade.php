...
@if($propertyId)
    <form id="contact-form" autocomplete="off" action="{{route('contact.property', $propertyId)}}" method="{{ $method }}" class="contact-form validateForm">
@else
    <form id="contact-form" autocomplete="off" action="{{ route('contact.send') }}" method="post" class="validateForm">
@endif
    @csrf
    @if($investmentId)
